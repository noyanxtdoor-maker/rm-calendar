...
M4 remote sync has not begun, but its local prerequisites are now real and tested: typed operation kinds, operation base revisions/dependencies, canonical payload preparation, safe acknowledgement/retry/conflict handling, and a privacy-safe Sync Status surface. The local Supabase RPC is proven for idempotent Contact/Household/Place and Activity/Quick Capture/Task/Note creation, plus revision-checked Activity update/complete/reopen and Task completion. Compound Follow-ups, browser authentication, and a hosted project remain deliberately unfinished. The detailed boundary and external authority requirements are in [Sync-Contract.md](Sync-Contract.md) and [Phase-4-Milestone-4-Prep.md](Phase-4-Milestone-4-Prep.md).
...
| M4 — Auth/sync | Local operation contract/preflight plus versioned identity/domain/RLS migration foundation, owner-isolation pgTAP tests, idempotent creation, and revision-checked Activity/Task transition RPC operations complete; atomic follow-up apply, authenticated transport, and a hosted owner workspace remain | Requires a new dedicated RM Calendar Supabase project; then two signed-in browser profiles sync exactly once and intentional conflict is visible with no data loss |
