...
M4 remote sync has not begun, but its local prerequisites are now real and tested: typed operation kinds, operation base revisions/dependencies, canonical payload preparation, safe acknowledgement/retry/conflict handling, and a privacy-safe Sync Status surface. The local Supabase RPC is now additionally proven for idempotent Contact/Household/Place and Activity/Quick Capture/Task/Note creation. Remote lifecycle transitions, compound Follow-ups, browser authentication, and a hosted project remain deliberately unfinished. The detailed boundary and external authority requirements are in [Sync-Contract.md](Sync-Contract.md) and [Phase-4-Milestone-4-Prep.md](Phase-4-Milestone-4-Prep.md).
...
| M4 — Auth/sync | Local operation contract/preflight plus versioned identity/domain/RLS migration foundation, owner-isolation pgTAP tests, and restricted idempotent create operations for Contact/Household/Place/Activity/Quick Capture/Task/Note complete; lifecycle transitions/follow-up apply, authenticated transport, and a hosted owner workspace remain | Requires a new dedicated RM Calendar Supabase project; then two signed-in browser profiles sync exactly once and intentional conflict is visible with no data loss |
