...
| M4 — Auth/sync | Local operation contract/preflight plus versioned identity/domain/RLS migration foundation and owner-isolation pgTAP test complete; `apply_sync_batch`, authenticated transport, and a hosted owner workspace remain | Requires a new dedicated RM Calendar Supabase project; then two signed-in browser profiles sync exactly once and intentional conflict is visible with no data loss |
