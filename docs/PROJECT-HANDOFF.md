...
| M4 — Auth/sync | Local operation contract/preflight plus versioned identity/domain/RLS migration foundation complete; `pull_changes`/`apply_sync_batch`, database execution, owner workspace, and authenticated transport remain | Requires a new dedicated RM Calendar Supabase project and a local Docker-backed database test run; then two signed-in browser profiles sync exactly once and intentional conflict is visible with no data loss |
