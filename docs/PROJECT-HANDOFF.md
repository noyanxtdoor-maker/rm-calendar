...
### Phase 4 — Milestone 3: completion, outcome, tasks, and follow-up: complete
...
M4’s complete **local sync contract** is now proven: typed operation kinds, operation base revisions/dependencies, canonical payload preparation, safe acknowledgement/retry/conflict handling, and a privacy-safe Sync Status surface. The local Supabase RPC covers idempotent Contact/Household/Place and Activity/Quick Capture/Task/Note creation, revision-checked Activity update/complete/reopen and Task completion, and atomic Task/Activity Follow-ups with target-aware acknowledgement. Browser authentication, a remote adapter, a dedicated hosted project, and two-profile remote proof remain deliberately unfinished. The detailed boundary and external authority requirements are in [Sync-Contract.md](Sync-Contract.md) and [Phase-4-Milestone-4-Prep.md](Phase-4-Milestone-4-Prep.md).
...
4. Present the implemented local acknowledgement plus final production onboarding consent and independent/non-affiliation language.
5. Retain the implemented local export/erase controls and add account-delete request behavior after authentication exists.
...
| M4 — Auth/sync | Local operation contract/preflight plus versioned identity/domain/RLS migration foundation, owner-isolation pgTAP tests, idempotent creation, revision-checked Activity/Task transitions, and atomic Follow-up RPC operations complete; authenticated transport and a hosted owner workspace remain | Requires a new dedicated RM Calendar Supabase project; then two signed-in browser profiles sync exactly once and intentional conflict is visible with no data loss |
| M5 — Beta readiness | Local export/erase/persistence/onboarding preparation complete; account-aware sign-out/delete, deployment/auth config, and manual beta script remain | The local privacy/recovery checklist passes with fictional data; no public invite before final approval |
...
- The Phase 0-2 documents and active prototype were published previously; Phase 3 plans and the Phase 4 Milestones 0-3 implementation/evidence are the current publication set.
...
## 11. Current execution gate: external authorization required for M4
...
Milestones 1, 2, and 3 are complete. M4 local preparation and M5 local privacy/recovery preparation are also complete, but M4 authenticated sync is not started. Do not create a hosted Supabase project, configure email, store beta data remotely, invite real users, use a real domain, or implement a real remote adapter without separate founder direction.
...
1. create/use the named Supabase project and its environment boundaries;
2. implement schema migrations and RLS tests before any real sign-in UI;
3. configure the approved email/OTP provider and redirect policy;
4. implement remote RPC and outbox sync only after those access controls pass;
5. use fictional or explicitly consented data in every test and demo.
